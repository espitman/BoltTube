...
struct ContentView: View {
    ...
    var body: some View {
        ...
        } message: { Text("Are you sure? This action is permanent.") }
    }
                ...
                VStack(alignment: .leading, spacing: 36) {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        HStack(spacing: 0) {
                            ...
                            Button { if let s = NSPasteboard.general.string(forType: .string) { controller.videoURL = s.trimmingCharacters(in: .whitespacesAndNewlines); controller.scheduleQualityRefresh() } } label: { Text("Paste").font(.system(size: 14, weight: .bold)).foregroundStyle(Color.white).padding(.horizontal, 28).padding(.vertical, 14) }.buttonStyle(.plain).background(accentRed)
    ...
    private func channelsGrid(metrics: LayoutMetrics) -> some View {
        ScrollView(.vertical, showsIndicators: false) {
            if controller.channels.isEmpty {
                VStack(spacing: 20) { Image(systemName: "square.grid.2x2").font(.system(size: 60)).foregroundStyle(slate600.opacity(0.1)); Text("No channels yet").font(.system(size: 16, weight: .medium)).foregroundStyle(slate600.opacity(0.4)); Button("Create your first channel") { showCreateChannel = true }.buttonStyle(.link) }.frame(maxWidth: .infinity).padding(.top, 100)
            } else {
                LazyVGrid(columns: [GridItem(.adaptive(minimum: 180, maximum: 240), spacing: 24)], spacing: 32) {
    ...
    private func playlistDetailView(playlist: Playlist) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 12) {
                Button { controller.selectedPlaylist = nil; controller.playlistItems = [] } label: { Image(systemName: "arrow.left").font(.system(size: 16, weight: .bold)).foregroundStyle(slate900).padding(10).background(Color.white).clipShape(Circle()).shadow(color: Color.black.opacity(0.05), radius: 5) }.buttonStyle(.plain)
                Text(playlist.name).font(.system(size: 18, weight: .bold)).foregroundStyle(slate900)
                Spacer()
    ...
    private func channelDetailView(channel: Channel) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 12) {
                Button { controller.selectedChannel = nil; controller.channelPlaylists = [] } label: { Image(systemName: "arrow.left").font(.system(size: 16, weight: .bold)).foregroundStyle(slate900).padding(10).background(Color.white).clipShape(Circle()).shadow(color: Color.black.opacity(0.05), radius: 5) }.buttonStyle(.plain)
                Text(channel.name).font(.system(size: 18, weight: .bold)).foregroundStyle(slate900)
                Spacer()
            }.padding(.horizontal, 40).padding(.bottom, 24)
            Text("Coming Soon").padding(.horizontal, 40).foregroundStyle(slate600)
        }
    }
    ...
    private func previewArea() -> some View {
        HStack(spacing: 24) {
            ZStack(alignment: .bottomTrailing) {
                ...
                if controller.resolvedDurationSeconds > 0 { Text(String(format: "%d:%02d", controller.resolvedDurationSeconds/60, controller.resolvedDurationSeconds%60)).font(.system(size: 11, weight: .bold)).padding(.horizontal, 8).padding(.vertical, 4).background(Color.black.opacity(0.75)).foregroundStyle(Color.white).clipShape(RoundedRectangle(cornerRadius: 6)).padding(8) }
    ...
    private func downloadControls() -> some View {
        VStack(alignment: .leading, spacing: 16) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                else { ScrollView(.horizontal, showsIndicators: false) { HStack(spacing: 8) { ForEach(controller.formats) { f in let isS = controller.selectedFormatID == f.id; Button { controller.selectedFormatID = f.id } label: { VStack(spacing: 2) { Text(f.title).font(.system(size: 13, weight: isS ? .black : .bold)).foregroundStyle(isS ? Color.white : slate900); if !f.filesize.isEmpty { Text(f.filesize).font(.system(size: 10, weight: .medium)).foregroundStyle(isS ? Color.white.opacity(0.8) : slate600) } }.padding(.horizontal, 14).padding(.vertical, 8).background(RoundedRectangle(cornerRadius: 8).fill(isS ? accentBlue : Color.white).overlay(RoundedRectangle(cornerRadius: 8).stroke(isS ? accentBlue : slate900.opacity(0.12), lineWidth: 1))) }.buttonStyle(.plain) } } } }
            }
            ...
            else { Button { Task { await controller.downloadVideo() } } label: { HStack(spacing: 8) { Image(systemName: "arrow.down"); Text("Download") }.font(.system(size: 14, weight: .bold)).foregroundStyle(Color.white).frame(maxWidth: .infinity).frame(height: 46).background(controller.isResolvingQualities || controller.formats.isEmpty ? Color.gray.opacity(0.1) : accentBlue).clipShape(RoundedRectangle(cornerRadius: 12)) }.buttonStyle(.plain) }
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Button(action: onPlay) {
                ...
                .overlay(alignment: .bottomTrailing) { if item.duration > 0 { Text(formatDuration(item.duration)).font(.system(size: 11, weight: .bold)).padding(.horizontal, 8).padding(.vertical, 4).background(Color.black.opacity(0.8)).foregroundStyle(Color.white).clipShape(RoundedRectangle(cornerRadius: 6)).padding(8) } }
...
struct AddToChannelModal: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack { Text("Add to Channel").font(.system(size: 16, weight: .bold)); Spacer(); Button { dismiss() } label: { Image(systemName: "xmark").font(.system(size: 14)).foregroundStyle(Color.gray) }.buttonStyle(.plain) }.padding(24); Divider()
            ScrollView { VStack(spacing: 8) { if controller.channels.isEmpty { Text("No channels found").font(.system(size: 14)).foregroundStyle(Color.gray).padding(40) } else { ForEach(controller.channels) { c in Button { Task { await controller.addPlaylistToChannel(channelID: c.id, playlistID: playlist.id); dismiss() } } label: { HStack(spacing: 12) { ZStack { if let thumb = c.thumbnailUrl, !thumb.isEmpty { AsyncImage(url: URL(string: thumb)) { phase in if let image = phase.image { image.resizable().aspectRatio(contentMode: .fill) } else { Color.gray.opacity(0.1) } } } else { Color.gray.opacity(0.1) } }.frame(width: 40, height: 30).clipShape(RoundedRectangle(cornerRadius: 4)); Text(c.name).font(.system(size: 14, weight: .medium)).foregroundStyle(Color(red: 0.07, green: 0.09, blue: 0.15)); Spacer(); Image(systemName: "plus.square.fill").foregroundStyle(Color.blue) }.padding(.horizontal, 20).padding(.vertical, 12).background(Color.black.opacity(0.02)).clipShape(RoundedRectangle(cornerRadius: 10)) }.buttonStyle(.plain) } } }.padding(24) }
        }.frame(width: 320, height: 400).background(Color.white)
    }
}
