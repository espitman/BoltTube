...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $itemToAddToPlaylist) { item in
            AddToPlaylistModal(controller: controller, item: item)
        }
        ...
        .alert("Delete File?", isPresented: Binding(
            ...
            Button("Delete", role: .destructive) { if let item = itemToDelete { deleteItem(item) } }
    ...
    private func deleteItem(_ item: MediaLibraryItem) {
        Task { await controller.deleteItem(id: item.id); itemToDelete = nil }
    }

    private func closePlayer() {
        playingItem = nil; player?.pause(); player = nil
    }
    ...
    private func downloadControls() -> some View {
        VStack(alignment: .leading, spacing: 16) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                else { ScrollView(.horizontal, showsIndicators: false) { HStack(spacing: 8) { ForEach(controller.formats) { format in let isSelected = controller.selectedFormatID == format.id; Button { controller.selectedFormatID = format.id } label: { VStack(spacing: 2) { Text(format.title).font(.system(size: 13, weight: isSelected ? .black : .bold)).foregroundStyle(isSelected ? .white : slate900); if !format.filesize.isEmpty { Text(format.filesize).font(.system(size: 10, weight: .medium)).foregroundStyle(isSelected ? .white.opacity(0.8) : slate600) } }.padding(.horizontal, 14).padding(.vertical, 8).background(RoundedRectangle(cornerRadius: 8).fill(isSelected ? accentBlue : Color.white).overlay(RoundedRectangle(cornerRadius: 8).stroke(isSelected ? accentBlue : slate900.opacity(0.12), lineWidth: 1))) }.buttonStyle(.plain) } } } }
            }
    ...
    private func playerOverlay() -> some View {
        ZStack {
            ...
            VStack(spacing: 0) { if let p = player { VideoPlayer(player: p).clipShape(RoundedRectangle(cornerRadius: 12)).overlay(alignment: .topTrailing) { Button { closePlayer() } label: { Image(systemName: "xmark.circle.fill").font(.system(size: 24)).foregroundStyle(.white.opacity(0.7)).padding(16) }.buttonStyle(.plain) } } }.frame(width: 800, height: 480).background(Color.black).clipShape(RoundedRectangle(cornerRadius: 16)).shadow(color: .black.opacity(0.3), radius: 30)
        }
    }
...
struct VideoCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Button(action: onPlay) {
                ZStack {
                    if let thumb = item.thumbnailUrl, !thumb.isEmpty { AsyncImage(url: URL(string: thumb)) { phase in if let image = phase.image { image.resizable().aspectRatio(contentMode: .fill) } else { Color.black.overlay { ProgressView().scaleEffect(0.5) } } } }
                ...
                .overlay(alignment: .bottomTrailing) { if item.duration > 0 { Text(formatDuration(item.duration)).font(.system(size: 11, weight: .bold)).padding(.horizontal, 8).padding(.vertical, 4).background(.black.opacity(0.8)).foregroundStyle(.white).clipShape(RoundedRectangle(cornerRadius: 6)).padding(8).zIndex(10) } }
    ...
    private func formatDuration(_ seconds: Int) -> String { let m = seconds / 60; let s = seconds % 60; return String(format: "%d:%02d", m, s) }
}
...
struct PlaylistCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ZStack {
                if let thumb = playlist.thumbnailUrl, !thumb.isEmpty { AsyncImage(url: URL(string: thumb)) { phase in if let image = phase.image { image.resizable().aspectRatio(contentMode: .fill) } else { Color.gray.opacity(0.1).overlay { Image(systemName: "music.note.list").foregroundStyle(.gray.opacity(0.3)).font(.system(size: 30)) } } } }
                else { Color.gray.opacity(0.1).overlay { Image(systemName: "music.note.list").foregroundStyle(.gray.opacity(0.3)).font(.system(size: 30)) } }
            }.frame(height: 120).clipShape(RoundedRectangle(cornerRadius: 12)).shadow(color: .black.opacity(0.05), radius: 10, y: 5)
            VStack(alignment: .leading, spacing: 2) { Text(playlist.name).font(.system(size: 14, weight: .bold)).foregroundStyle(Color(red: 0.07, green: 0.09, blue: 0.15)).lineLimit(1); Text("Playlist").font(.system(size: 11, weight: .medium)).foregroundStyle(Color.gray.opacity(0.6)) }.padding(.horizontal, 4)
        }
    }
}
...
struct RecentCardCompact: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Button(action: onPlay) {
                ZStack { if let thumb = item.thumbnailUrl, !thumb.isEmpty { AsyncImage(url: URL(string: thumb)) { phase in if let image = phase.image { image.resizable().aspectRatio(contentMode: .fill) } else { Color.gray.opacity(0.1) } } } else { RoundedRectangle(cornerRadius: 8).fill(Color.gray.opacity(0.1)) } }.frame(width: 80, height: 50).clipShape(RoundedRectangle(cornerRadius: 8))
                .overlay(alignment: .bottomTrailing) { if item.duration > 0 { Text(formatDuration(item.duration)).font(.system(size: 8, weight: .bold)).padding(.horizontal, 4).padding(.vertical, 2).background(.black.opacity(0.8)).foregroundStyle(.white).clipShape(RoundedRectangle(cornerRadius: 4)).padding(4).zIndex(10) } }
                .overlay { if controller.refreshingIDs.contains(item.id) { ZStack { Color.black.opacity(0.4); ProgressView().controlSize(.small).tint(.white).scaleEffect(0.6) }.clipShape(RoundedRectangle(cornerRadius: 8)) } }
            }.buttonStyle(.plain)
...
struct AddToPlaylistModal: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(spacing: 8) {
                    if controller.playlists.isEmpty { Text("No playlists found").font(.system(size: 14)).foregroundStyle(.gray).padding(40) }
                    else { ForEach(controller.playlists) { playlist in Button { Task { await controller.addToPlaylist(playlistID: playlist.id, mediaID: item.id); dismiss() } } label: { HStack(spacing: 12) { ZStack { if let thumb = playlist.thumbnailUrl, !thumb.isEmpty { AsyncImage(url: URL(string: thumb)) { phase in if let image = phase.image { image.resizable().aspectRatio(contentMode: .fill) } else { Color.gray.opacity(0.1) } } } else { Color.gray.opacity(0.1) } }.frame(width: 40, height: 30).clipShape(RoundedRectangle(cornerRadius: 4)); Text(playlist.name).font(.system(size: 14, weight: .medium)).foregroundStyle(Color(red: 0.07, green: 0.09, blue: 0.15)); Spacer(); Image(systemName: "plus.circle").foregroundStyle(.blue) }.padding(.horizontal, 20).padding(.vertical, 12).background(Color.black.opacity(0.02)).clipShape(RoundedRectangle(cornerRadius: 10)) }.buttonStyle(.plain) } }
                }.padding(24)
            }
...
struct ShimmerModifier: ViewModifier {
    ...
    func body(content: Content) -> some View { content.overlay(LinearGradient(gradient: Gradient(colors: [.clear, .white.opacity(0.45), .clear]), startPoint: .init(x: phase, y: 0.5), endPoint: .init(x: phase + 0.8, y: 0.5)).blendMode(.plusLighter)).onAppear { withAnimation(.linear(duration: 1.3).repeatForever(autoreverses: false)) { phase = 1.5 } } }
}
