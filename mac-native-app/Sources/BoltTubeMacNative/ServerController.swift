...
struct RemoteFormat: Codable, Identifiable, Hashable {
    let id: String; let title: String; let details: String; let filesize: String
}

struct DownloadTask: Identifiable, Hashable {
    let id: String; let title: String; var status: String; var progress: Double
}

struct ResolveResponse: Codable {
    let title: String; let thumbnailUrl: String; let durationSeconds: Int; let formats: [RemoteFormat]
}
...
final class ServerController {
    ...
    init() {
        let defaultDirectory = FileManager.default.homeDirectoryForCurrentUser.appending(path: "Movies/BoltTubeNative", directoryHint: .isDirectory)
        self.downloadDirectory = defaultDirectory
        ensureDirectoryExists(defaultDirectory)
        
        // CORRECTION: Start server FIRST, then sync data
        Task {
    ...
    func chooseDownloadDirectory() {
        ...
        panel.canChooseFiles = false; panel.canChooseDirectories = true; panel.allowsMultipleSelection = false; panel.prompt = "Select"
        if panel.runModal() == .OK, let url = panel.url {
            downloadDirectory = url; ensureDirectoryExists(url)
            appendLog("Library folder set to \(url.path)"); Task { await refreshLibrary() }
        }
    }
    ...
    func pasteFromClipboard() {
        ...
        let value = pb.string(forType: .string) ?? pb.string(forType: .URL) ?? pb.string(forType: NSPasteboard.PasteboardType("public.url"))
        if let value = value {
            let trimmed = value.trimmingCharacters(in: .whitespacesAndNewlines)
            appendLog("Pasted: \(trimmed.prefix(60))..."); videoURL = trimmed; scheduleQualityRefresh()
        }
    }
    ...
    func scheduleQualityRefresh() {
        ...
        if trimmedURL.isEmpty { resolvedTitle = ""; formats = []; selectedFormatID = "best"; return }
    ...
    func downloadVideo() async {
        ...
        do {
            ...
            lastDownloadedFileName = response.fileName; downloadProgress = 1; downloadProgressText = "Download complete"
            appendLog("Saved \(response.fileName)"); await refreshLibrary(); videoURL = ""; resolvedTitle = ""; resolvedThumbnailUrl = ""; resolvedDurationSeconds = 0; formats = []; selectedFormatID = "best"
    ...
    func cancelDownload() {
        if let process = activeDownloadProcess {
            let pid = process.processIdentifier; process.terminate()
        ...
        if !tempName.isEmpty {
            ...
            if let files = try? fm.contentsOfDirectory(atPath: downloadDirectory.path) {
                for file in files where file.hasPrefix(tempName) {
                    let url = downloadDirectory.appendingPathComponent(file); try? fm.removeItem(at: url); appendLog("Cleaned up partial file: \(file)")
                }
            }
        }
    ...
    func refreshMetadata(id: String) async {
        ...
        do {
            ...
            let (_, response) = try await URLSession.shared.data(for: request)
            guard let httpResponse = response as? HTTPURLResponse, (200..<300).contains(httpResponse.statusCode) else { return }
            appendLog("Metadata refreshed."); await refreshLibrary()
        } catch { appendLog("Refresh failed.") }
    }
    ...
    func refreshLibrary() async {
        do {
            let data = try await runJSONCommand(arguments: [bridgeScriptURL.path, "list", "--download-dir", downloadDirectory.path], logOutput: false)
            let decoder = JSONDecoder(); decoder.keyDecodingStrategy = .convertFromSnakeCase
            let decoded = try decoder.decode(MediaLibraryResponse.self, from: data)
            libraryItems = decoded.items
        } catch { appendLog("Library refresh failed.") }
    }
    ...
    func refreshPlaylists() async {
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse, (200..<300).contains(httpResponse.statusCode) else { return }
            let decoder = JSONDecoder(); decoder.keyDecodingStrategy = .convertFromSnakeCase
            let decoded = try decoder.decode(PlaylistResponse.self, from: data)
            playlists = decoded.items
        } catch { appendLog("Playlists refresh failed.") }
    }
    ...
    func createPlaylist(name: String) async {
        ...
        let body = ["name": name]; request.httpBody = try? JSONSerialization.data(withJSONObject: body)
    ...
    func addToPlaylist(playlistID: Int, mediaID: String) async {
        ...
        var request = URLRequest(url: url); request.httpMethod = "POST"; request.addValue("application/json", forHTTPHeaderField: "Content-Type")
        let body: [String: Any] = ["playlistId": playlistID, "mediaId": mediaID]; request.httpBody = try? JSONSerialization.data(withJSONObject: body)
    ...
    func updatePlaylist(id: Int, name: String) async {
        ...
        var request = URLRequest(url: url); request.httpMethod = "POST"; request.addValue("application/json", forHTTPHeaderField: "Content-Type")
        let body: [String: Any] = ["id": id, "name": name]; request.httpBody = try? JSONSerialization.data(withJSONObject: body)
    ...
    func deletePlaylist(id: Int) async {
        ...
        var request = URLRequest(url: url); request.httpMethod = "POST"; request.addValue("application/json", forHTTPHeaderField: "Content-Type")
        let body = ["id": id]; request.httpBody = try? JSONSerialization.data(withJSONObject: body)
    ...
    func startShareServer() async {
        ...
        process.executableURL = venvPythonURL; process.arguments = [bridgeScriptURL.path, "serve", "--port", normalizedPort, "--download-dir", downloadDirectory.path]
        process.environment = mergedEnvironment(extra: ["PYTHONUNBUFFERED": "1"])
        ...
        do {
            appendLog("Starting share server..."); try process.run(); shareServerProcess = process; shareServerOutputPipe = outputPipe; isShareServerRunning = true
            _ = IOPMAssertionCreateWithDescription(kIOPMAssertionTypeNoIdleSleep as CFString, "BoltTube Bridge Server Running" as CFString, nil, nil, nil, 0, nil, &sleepAssertionID)
            try await Task.sleep(for: .seconds(2)); await checkHealth()
        } catch { appendLog("Failed to start server."); outputPipe.fileHandleForReading.readabilityHandler = nil; shareServerProcess = nil; shareServerOutputPipe = nil }
    }

    func stopShareServer() {
        shareServerOutputPipe?.fileHandleForReading.readabilityHandler = nil; shareServerProcess?.terminate(); shareServerProcess = nil; shareServerOutputPipe = nil; isShareServerRunning = false
        if sleepAssertionID != 0 { IOPMAssertionRelease(sleepAssertionID); sleepAssertionID = 0 }
    }
    ...
    func checkHealth() async {
        ...
        do {
            ...
            lastHealthMessage = "Share server OK on port \(decoded.port)."; appendLog(lastHealthMessage)
        } catch { lastHealthMessage = "Health check failed."; appendLog(lastHealthMessage) }
    }

    private func ensurePythonReady() async -> Bool {
        if FileManager.default.fileExists(atPath: venvPythonURL.path) { return true }
        appendLog("Preparing Python..."); await installPythonEnvironment(); return FileManager.default.fileExists(atPath: venvPythonURL.path)
    }

    private func installPythonEnvironment() async {
        do {
            ensureDirectoryExists(appSupportDirectory); ensureDirectoryExists(downloadDirectory)
            if !FileManager.default.fileExists(atPath: venvPythonURL.path) {
                try await runCommand(executable: URL(fileURLWithPath: "/usr/bin/python3"), arguments: ["-m", "venv", venvDirectory.path])
            }
            try await runCommand(executable: venvPipURL, arguments: ["install", "-r", requirementsURL.path])
            appendLog("Python ready.")
        } catch { appendLog("Python setup failed.") }
    }

    private func resolveQualities(for url: String) async {
        guard await ensurePythonReady() else { return }
        isResolvingQualities = true; defer { isResolvingQualities = false }
        do {
            let data = try await runJSONCommand(arguments: [bridgeScriptURL.path, "resolve", "--download-dir", downloadDirectory.path, "--url", url], logOutput: false)
            let decoder = JSONDecoder(); decoder.keyDecodingStrategy = .convertFromSnakeCase
            ...
            resolvedTitle = response.title; resolvedThumbnailUrl = response.thumbnailUrl; resolvedDurationSeconds = response.durationSeconds; formats = response.formats; selectedFormatID = response.formats.last?.id ?? "best"
        } catch { appendLog("Quality load failed.") }
    }
    ...
    private func runJSONCommand(arguments: [String], logOutput: Bool = true) async throws -> Data {
        try await Task.detached { [venvPythonURL] in
            let process = Process(); let outputPipe = Pipe(); let errorPipe = Pipe()
            process.executableURL = venvPythonURL; process.arguments = arguments; process.environment = Self.staticMergedEnvironment(extra: ["PYTHONUNBUFFERED": "1"])
            process.standardOutput = outputPipe; process.standardError = errorPipe
            try process.run(); let outputData = try outputPipe.fileHandleForReading.readToEnd() ?? Data()
            process.waitUntilExit()
            if process.terminationStatus != 0 { throw NSError(domain: "BoltTube", code: Int(process.terminationStatus)) }
            return outputData
        }.value
    }
    ...
    private func runDownloadCommand(arguments: [String]) async throws -> Data {
        try await withCheckedThrowingContinuation { continuation in
            let process = Process(); let outputPipe = Pipe(); let errorPipe = Pipe(); let state = DownloadStreamState()
            process.executableURL = venvPythonURL; process.arguments = arguments; process.environment = mergedEnvironment(extra: ["PYTHONUNBUFFERED": "1"])
            process.standardOutput = outputPipe; process.standardError = errorPipe
            errorPipe.fileHandleForReading.readabilityHandler = { handle in
                let data = handle.availableData
                guard !data.isEmpty, let text = String(data: data, encoding: .utf8) else { return }
                let lines = state.append(text: text)
                Task { @MainActor [weak self] in for l in lines { self?.handleDownloadProgressLine(l) } }
            }
            process.terminationHandler = { _ in
                Task { @MainActor [weak self] in
                    self?.activeDownloadProcess = nil; self?.isDownloading = false
                    do { let d = try outputPipe.fileHandleForReading.readToEnd() ?? Data(); if state.markResumed() { continuation.resume(returning: d) } }
                    catch { if state.markResumed() { continuation.resume(throwing: error) } }
                }
            }
            do { try process.run(); Task { @MainActor [weak self] in self?.activeDownloadProcess = process } } 
            catch { if state.markResumed() { continuation.resume(throwing: error) } }
        }
    }
    ...
    private func runCommand(executable: URL, arguments: [String]) async throws {
        try await Task.detached {
            ...
            process.executableURL = executable; process.arguments = arguments; process.environment = Self.staticMergedEnvironment()
    ...
    private func handleDownloadProgressLine(_ line: String) {
        ...
        if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any], let event = json["event"] as? String {
            if event == "progress", let frac = json["fraction"] as? Double { downloadProgress = frac; downloadProgressText = "\(Int(frac * 100))%" }
            else if event == "starting" { appendLog("Starting..."); if let t = json["tempName"] as? String { activeDownloadTempName = t } }
            else if event == "merging" { downloadProgressText = "Merging..."; appendLog("Processing final file...") }
        }
    }
            ...
            while ptr != nil {
                ...
                if interface?.ifa_addr.pointee.sa_family == UInt8(AF_INET) {
                    ...
                    if name == "en0" || name == "en1" {
                        ...
                        address = String(decoding: hostname.filter { $0 != 0 }.map { UInt8(bitPattern: $0) }, as: UTF8.self)
                    }
                }
            }
            freeifaddrs(ifaddr)
