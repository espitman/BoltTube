...
final class ServerController {
    ...
    func updateChannel(id: Int, name: String) async {
        ...
        do { 
            let (data, response) = try await URLSession.shared.data(for: request)
            if let r = response as? HTTPURLResponse { appendLog("Update channel response: \(r.statusCode)") }
            await refreshChannels() 
        } catch { 
            appendLog("Update channel failed: \(error.localizedDescription)") 
        }
    }
    ...
    private func formatSpeed(bytesPerSecond: Double) -> String { if bytesPerSecond >= 1024 * 1024 { return String(format: "%.1f MB", bytesPerSecond / (1024 * 1024)) }; if bytesPerSecond >= 1024 { return String(format: "%.0f KB", bytesPerSecond / 1024) }; return String(format: "%.0f B", bytesPerSecond) }
